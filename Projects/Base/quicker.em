...
macro AutoExpand()
{
    ...
    if(language != 0)
    {
    	/*we change to use Chinese at default*/
        language = 0
    }
    nVer = 0
    nVer = GetVersion()
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    {
        DelBufLine(hbuf, ln)
        szStructName = tolower(Ask("Please input struct name"))
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    {
        DelBufLine(hbuf, ln)
        szStructName = tolower(Ask("请输入结构名:"))
        InsBufLine(hbuf, ln, "@szLine1@typedef struct");
        InsBufLine(hbuf, ln + 1, "@szLine1@{");
        InsBufLine(hbuf, ln + 2, "@szLine@      ");
        szStructName = cat(szStructName,"_t")
    ...
    else if (szCmd == "cs")
    {
        ...
        if(strlen(szQuestion)>0)
        { 
        	InsBufLine(hbuf, ln, "@szLine1@/* comment by @szMyName@, @sz@-@szMonth@-@szDay@, Mantis号:@szQuestion@, 原因: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* comment by @szMyName@, @sz@-@szMonth@-@szDay@, 原因: */");        
        }
        return
    }
    else
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    sz = GetFileName(GetBufName (hbuf))
    InsBufLine(hbuf, ln + 1,  "//")
    InsBufLine(hbuf, ln + 2,  "// 版权声明： Copyright (c) 2020 @szName@. All rights reserved.")
    InsBufLine(hbuf, ln + 3,  "// 文件名称： @sz@")
    iLen = strlen (szContent)
    nlnDesc = ln
    szTmp = "// 文件摘要： "
    InsBufLine(hbuf, ln + 4, "// 文件摘要： @szContent@")
    InsBufLine(hbuf, ln + 5, "// 创建作者： @szName@")
    InsBufLine(hbuf, ln + 6, "// 创建时间： @szYear@-@szMonth@-@szDay@")
    ...
    //自动排列显示功能描述
    CommentContent(hbuf,nlnDesc+4,"// 文件摘要： ",szContent,0)
}
